...
/**
 * Checks a decoded request body against a generated specification interface. Everything it enforces
 * comes from the interface itself: a getter that cannot return null is a required field, its return
 * type is the field's type, its value constants are the values allowed, and the CONSTRAINTS constant
 * carries the rest of what the schema said.
 */
class RequestValidator
{
     ...
     *
     * @param ReflectionMethod $method
     * @return class-string|null Fully qualified class/interface name or null
     */
     ...
     * @return class-string|null Fully qualified class/interface name or null if not found
     */
     ...
     * @return class-string|null Fully qualified name or null
     */
    ...
    /**
     * The schema rules the generator carried onto the interface for this field, if it declared any.
     *
     * @param ReflectionMethod $method
     * @return array<string, mixed>
     */
    private function rulesFor(ReflectionMethod $method): array
    {
        $constraints = $method->getDeclaringClass()->getConstants()[ConstraintChecker::CONSTANT] ?? null;

        if (!is_array($constraints)) {
            return [];
        }

        $rules = $constraints[$this->methodToKey($method->getName())] ?? null;
